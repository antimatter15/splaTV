...
function createWorker(self) {
  ...
  function runSort(viewProj) {
    ...
    for (let i = 0; i < vertexCount; i++) {
      let depth =
        ((viewProj[2] * f_buffer[3 * i + 0] +
          viewProj[6] * f_buffer[3 * i + 1] +
          viewProj[10] * f_buffer[3 * i + 2]) *
          4096) |
        0;
      sizeList[i] = depth;
  ...
  function processPlyBuffer(inputBuffer) {
    ...
    for (let j = 0; j < vertexCount; j++) {
      ...
      // rgb
      texdata_c[4 * (16 * j + 7) + 0] = Math.max(
        0,
        Math.min(255, attrs.f_dc_0 * 255)
      );
      texdata_c[4 * (16 * j + 7) + 1] = Math.max(
        0,
        Math.min(255, attrs.f_dc_1 * 255)
      );
      texdata_c[4 * (16 * j + 7) + 2] = Math.max(
        0,
        Math.min(255, attrs.f_dc_2 * 255)
      );

      // opacity
      texdata_c[4 * (16 * j + 7) + 3] =
      ...
      texdata[16 * j + 8 + 7] = packHalf2x16(
        attrs.trbf_center,
        Math.exp(attrs.trbf_scale)
      );
    }
    ...
    new Response(
      new Response(texdata.buffer).body.pipeThrough(
...
let defaultViewMatrix = [
  0.98, 0.05, 0.18, 0, -0.06, 0.99, 0.1, 0, -0.17, -0.11, 0.98, 0, 2.04, 0.04,
  5.54, 1,
];
...
async function main() {
  ...
  const frame = (now) => {
    ...
    if (vertexCount > 0) {
      ...
      gl.uniform1f(u_time, Math.sin(Date.now() / 1000) / 2 + 0.5);
